...
export class AdminUser extends Component {
  ...
  handleApprove = (id) => {
    Service.approveUsers(id)
      .then((response) => {
        Alert.success('User Approved', response.data);
        setTimeout(() => {
          window.location.reload();
        }, 1000);
      })
      .catch((error) => {
        Alert.error('Error Approving User', error);
        setTimeout(() => {
          window.location.reload();
        }, 1000);
      });
  ...
  handleReject = (id) => {
    Service.rejectUsers(id)
      .then((response) => {
        Alert.success('User Rejected', response.data);
        setTimeout(() => {
          window.location.reload();
        }, 1000);
      })
      .catch((error) => {
        Alert.error('Error Rejecting User', error);
        setTimeout(() => {
          window.location.reload();
        }, 1000);
      });
